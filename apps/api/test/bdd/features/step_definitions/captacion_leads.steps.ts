...
class MockVentasRepository implements IVentasRepository {
  ...
  async listarLeadsPorAsesor(): Promise<Lead[]> {
    return [];
  }
  ...
  async listarLeadsPorEstado(): Promise<Lead[]> {
    return [];
  }
...
When(
  "un nuevo prospecto {string} con correo {string} solicita informacion sobre ventas",
  async function (nombre: string, email: string) {
