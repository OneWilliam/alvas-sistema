...
export class ActualizarCitaUseCase implements CasoDeUso<ActualizarCitaInput, Resultado<void, ErrorDeDominio>> {
  ...
  async ejecutar(input: ActualizarCitaInput): Promise<Resultado<void, ErrorDeDominio>> {
    try {
      ...
      if (!lead) return resultadoFallido(new ErrorDeDominio("Lead no encontrado", { codigo: "LEAD_NOT_FOUND" }));

      const cita = lead.citas.find(c => c.id as string === input.idCita);
      if (!cita) return resultadoFallido(new ErrorDeDominio("Cita no encontrada", { codigo: "CITA_NOT_FOUND" }));
      ...
      await this.repository.registrarActividad(lead.id, "CITA_ACTUALIZADA", `Cita ${input.idCita} actualizada`);
