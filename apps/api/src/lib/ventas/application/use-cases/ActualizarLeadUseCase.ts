...
export class ActualizarLeadUseCase implements CasoDeUso<ActualizarLeadInput, Resultado<void, ErrorDeDominio>> {
  ...
  async ejecutar(input: ActualizarLeadInput): Promise<Resultado<void, ErrorDeDominio>> {
    try {
      ...
      await this.repository.registrarActividad(lead.id, "LEAD_ACTUALIZADO", "Datos del lead actualizados");
