...
export class ActualizarUsuarioUseCase implements CasoDeUso<ActualizarUsuarioInput, Resultado<ActualizarUsuarioOutput, ErrorDeDominio>> {
  constructor(private readonly usuarioRepository: IUsuarioRepository) {}

  async ejecutar(input: ActualizarUsuarioInput): Promise<Resultado<ActualizarUsuarioOutput, ErrorDeDominio>> {
    try {
      const usuario = await this.usuarioRepository.obtenerPorId(new IdUsuario(input.idUsuario));
      if (!usuario) {
        return resultadoFallido(new ErrorDeDominio("Usuario no encontrado", { codigo: "USER_NOT_FOUND" }));
      }
